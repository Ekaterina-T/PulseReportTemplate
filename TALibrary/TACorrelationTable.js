...
class TACorrelationTable{
    ...
    private function _render(){
        ...
        _createRows(_globals);
        _createColumns();
        _setupConditionalFormatting();
        _setupSupressing();
        _setupSorting();

        _table.TotalsFirst = true;
    }
    ...
    private function _getRowheadersExpression(){
        ...
        voeTemplateHeader += _taTableUtils.GetTAQuestionExpression('voeTemplate');

        if(voeTemplateHeader != "") {
            rowexpr = voeTemplateHeader;
        } else {
     ...
     * @instance
     * @private
     * @function _createRowsFromDimension
     * @description for voe template we have Dimension (categorization) header as a first row to calculate thresholds
     * then we have all questions from the dimension to calculate statistics for the list of categories
     */
    private function _createRows(_globals){

        var avg: HeaderFormula = new HeaderFormula();
        avg.Type = FormulaType.Expression;
        avg.Expression = 'AVERAGE(COLVALUES(2, ROWS))';
        avg.Decimals = 0;
        _table.RowHeaders.Add(avg);

        var headerID = ParamUtil.GetSelectedOptions(_globals, 'p_ImpactAnalysisDimension');

        for(var i=0; i<headerID.length; i++) {

            var header = TableUtil.getHeaderDescriptorObject(_globals, headerID[i]);

            if(header.Type === 'Question') {
                var qe: QuestionnaireElement = QuestionUtil.getQuestionnaireElement(_globals, header.Code);
                var row: HeaderQuestion = new HeaderQuestion(qe);
                row.IsCollapsed = true;
                row.DefaultStatistic = StatisticsType.Average;
                _table.RowHeaders.Add(row);
                ...
                categorization.DataSourceNodeId = DataSourceUtil.getDsId(_globals);
                ...
                _table.RowHeaders.Add(categorization);
            }
        }
